...
from llama_recipes.inference.model_utils import load_llama_from_config, load_llama_reward_from_config, load_auto_model_from_config
from llama_recipes.inference.model_utils import load_mistral_from_config, load_mistral_reward_from_config
...
def main(
    ...
    if 'rm_mse_' in fsdp_checkpoint_path:
        if 'mistral' in fsdp_checkpoint_path:
            model_def = load_mistral_reward_from_config(HF_model_path_or_name, 1)
        else:
            model_def = load_llama_reward_from_config(HF_model_path_or_name, 1)
    elif 'rm_ce_' in fsdp_checkpoint_path:
        if 'mistral' in fsdp_checkpoint_path:
            model_def = load_mistral_reward_from_config(HF_model_path_or_name, 2)
        else:
            model_def = load_llama_reward_from_config(HF_model_path_or_name, 2)
    elif 'ppo_rm' in fsdp_checkpoint_path:
    ...
    if 'ppo_rm' in fsdp_checkpoint_path:
